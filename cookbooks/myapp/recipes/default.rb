...
#################  removal complete ################
  ...
  db_settings = data_bag_item("jenkins", "settings")

  remote_file db_settings['rpm.local'] do
    source db_settings['rpmurl']
  end

  rpm_package 'jenkins' do
    source db_settings['rpm.local']
    action :install
  end
  
  node.override['jenkins_port'] = db_settings['port']
